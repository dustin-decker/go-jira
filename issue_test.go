...
func TestIssueService_SearchJQL_WithDocExample(t *testing.T) {
	...
	testMux.HandleFunc("/rest/api/3/issue/bulkfetch", func(w http.ResponseWriter, r *http.Request) {
		...
		// Check request body
		expectedBody := `{"issueIdsOrKeys":["10002"],"fields":["string"]}`
		bodyBytes, _ := io.ReadAll(r.Body)
		if diff := cmp.Diff(expectedBody, strings.TrimSpace(string(bodyBytes))); diff != "" {
			t.Errorf("Bulk Fetch Request body mismatch (-want +got):\n%s", diff)
		}
		...
		// Send response with full issue details
		w.WriteHeader(http.StatusOK)
		fmt.Fprint(w, `{"issues": [{"expand": "","id": "10002","self": "https://your-domain.atlassian.net/rest/api/3/issue/10002","key": "ED-1","fields": {"description": "Main order flow broken","comment": {"comments":[{"body":"Lorem ipsum dolor sit amet"}]},"attachment": [{"filename":"debuglog.txt"}]}}]}`)
	})

	// Create search options matching the example
	opt := &SearchOptions{
		MaxResults: 273,
		Fields:     []string{"string"},
	}

	// Call the Search method with the example parameters
	issues, resp, _, err := testClient.Issue.Search("string", opt)
	...
	if len(issues) != 1 {
		...
		return // Stop the test if we don't have the expected number of issues
	}

	// Verify key fields from the returned issue
	if issues[0].ID != "10002" || issues[0].Key != "ED-1" {
		t.Errorf("Issue data mismatch. Got ID=%s, Key=%s", issues[0].ID, issues[0].Key)
	}

	// Verify fields were populated correctly
	if issues[0].Fields == nil {
		t.Errorf("Expected Fields to be populated")
	} else {
		if issues[0].Fields.Description != "Main order flow broken" {
			t.Errorf("Expected description 'Main order flow broken', got '%s'", issues[0].Fields.Description)
		}
		if len(issues[0].Fields.Comments.Comments) != 1 {
			t.Errorf("Expected 1 comment, got %d", len(issues[0].Fields.Comments.Comments))
		}
		if len(issues[0].Fields.Attachments) != 1 {
			t.Errorf("Expected 1 attachment, got %d", len(issues[0].Fields.Attachments))
		}
	}

	// Note: We're not checking the next page token since we're ignoring it in the test
}
